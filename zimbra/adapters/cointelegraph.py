...
class CointelegraphAdapter(BaseAdapter):
    def fetch(self, target: str, limit: int = 50, **kwargs) -> List[PostRecord]:
        """Fetch news articles from CoinTelegraph tag RSS feed.
        ...
        try:
            ...
            for item in items[:limit]:
                ...
                author = creator_elem.text if creator_elem is not None else "CoinTelegraph"
                ...
                post_id = f"cointelegraph_{hashlib.md5(link.encode('utf-8', errors='ignore')).hexdigest()[:16]}"
            ...
            print(f"Error fetching from CoinTelegraph RSS: {e}")
